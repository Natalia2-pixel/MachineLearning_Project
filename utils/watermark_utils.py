...
from model_loader import download_and_extract_models_from_drive



zip_file_id = "1DN8GSYUkVaE5b6SaK4GJYjrhCja612Av"
download_and_extract_models_from_drive(zip_file_id)
...
def apply_watermark_dl_model(cover_image, watermark_image=None, model_type="CNN", alpha=0.3):
    ...
    if watermark_image is None:
        watermark_image = Image.new("L", (64, 64), color=0)
    cover = cover_image.convert("L").resize((64, 64))
    mark = watermark_image.convert("L").resize((64, 64)) if watermark_image else Image.new("L", (64, 64))
